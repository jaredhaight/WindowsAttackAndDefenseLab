...
function Get-ClassDeploymentStatus {
  ...
  $deployments = Get-AzResourceGroup | Get-AzResourceGroupDeployment | Select-Object ResourceGroupName, ProvisioningState
  if ($Failed)
  {
    return $deployments | Where-Object {$_.ProvisioningState -eq 'Failed'}
  }
  return $deployments
}

function Get-ClassStats {
  $cpus = 0
  $ram = 0
  ...
  "Standard_B2s"  {$cpus += 2; $ram += 4; break}
  "Standard_B2ms"   {$cpus += 2; $ram += 8; break}
  "Standard_B4ms" {$cpus += 4; $ram += 16; break}
  default {"Something else happened"; break}
      }
  }

  Write-Output "VMs: $($vms.Count)"
  Write-Output "vCPUs: $cpus"
